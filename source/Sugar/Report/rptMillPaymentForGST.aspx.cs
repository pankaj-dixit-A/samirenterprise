    ...
    protected void dtlList_OnItemDataBound(object sender, DataListItemEventArgs e)
    {
        ...
        string qryDetails = "select SUM(quantal) as qtl,mill_rate,excise_rate as gst_rate,(mill_rate + excise_rate) as totalmillrate," +
                             ...
                             ",isnull(PurchaseTDSRate,0) as TDSRate,(CAST((SUM(quantal) * (mill_rate)) as NUMERIC(18,2))*isnull(PurchaseTDSRate,0)/100) as TDSAmt " +
                              " ,(CAST((SUM(quantal) * (mill_rate + excise_rate)) as NUMERIC(18,2))+(CAST((SUM(quantal) * (mill_rate + excise_rate)) as NUMERIC(18,2))*isnull(TCS_Rate,0)/100)-(CAST((SUM(quantal) * (mill_rate)) as NUMERIC(18,2))*isnull(PurchaseTDSRate,0)/100)) as TCSNetPayable " +
                             ...
                             + " and Payment_To=" + millCode + " group by mill_rate,excise_rate,TCS_Rate,PurchaseTDSRate,amount,TDSAmt,doc_no";
